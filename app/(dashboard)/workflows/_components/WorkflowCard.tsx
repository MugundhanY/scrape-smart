...
import ExecutionsStatusIndicator, { ExecutionsStatusLabel } from '@/app/workflow/runs/[workflowId]/_components/ExecutionsStatusIndicator';
...
function LastRunDetails({workflow}: {workflow: Workflow}) {
    ...
    return <div className='bg-primary/5 px-4 py-1 flex items-center justify-between text-muted-foreground'>
        <div className='flex items-center text-sm gap-2'>
            {lastRunAt && (
                <Link href={`/workflow/runs/${workflow.id}/${lastRunId}`} className='flex items-center text-sm gap-2 group'>
                <span>Last run:</span>
                <ExecutionsStatusIndicator status={lastRunStatus as WorkflowExecutionStatus}/>
                <ExecutionsStatusLabel status={lastRunStatus as WorkflowExecutionStatus}/>
